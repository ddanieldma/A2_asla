...
O "mapa" do arquipélago foi feito no photoshop, com ilhas representativas que não são as reais retiradas do google maps, pois não conseguimos encontrar as a representação das ilhas certas. O espaço vago na direita da imagem foi deixado livre para que colocassemos os gráficos futuramente.
...
por_especie <- pg_size %>% group_by(species) %>% 
  summarize(contagem = n()) %>% 
  mutate(porcentagem = contagem / sum(contagem))
por_especie_e_ilha <- pg_size %>% group_by(island, species) %>%
  summarize(contagem = n()) %>%
  mutate(porcentagem = contagem / sum(contagem))

plot1_base <- 
  ggplot(por_especie, aes(x = species, y = porcentagem, fill = species)) +
  geom_col()
